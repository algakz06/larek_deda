...
class Contract(Base):
    ...
    customer_inn = mapped_column("customer_inn", VARCHAR(12), ForeignKey("users.inn"), nullable=False)
    ...
    subcontractor_sum_percents = mapped_column("subcontractor_sum_percents", MONEY, nullable=True)
    ...
    enforcement_type = mapped_column("enforcement_type", EnumType(EnforcementType), nullable=True)
    enforcement_amount_rub = mapped_column("enforcement_amount_rub", MONEY, nullable=True)
    supplier_inn = mapped_column("supplier_inn", VARCHAR(15), ForeignKey("users.inn"), nullable=False)
...
class ContractTermination(Base):
    ...
    contract_id = mapped_column("contract_id", TEXT, ForeignKey("contracts.contract_id"), primary_key=True)
    termination_date = mapped_column("termination_date", Date, nullable=False)
    termination_reason_info  = mapped_column("termination_reason_info", TEXT, nullable=False)
    termination_reason_name = mapped_column("termination_reason_name", TEXT, nullable=False)
...
class ContractImproperExecution(Base):
    ...
    execution_document_date = mapped_column("execution_document_date", Date, nullable=False)
    execution_document_name = mapped_column("execution_document_name", TEXT, nullable=False)
#endregion

#region notification
class NotificationInfo(Base):
    __tablename__ = "notification_info"
    procedure_id = mapped_column("procedure_id", TEXT, ForeignKey("contracts.procedure_id"), primary_key=True)
...
class Complaint(Base):
    ...
    procedure_id = mapped_column("procedure_id", TEXT, ForeignKey("contracts.procedure_id"), nullable=False)
...
class ParticipiantStat(Base):
    ...
    __table_args__ = (
        CheckConstraint('participiant_count >= wins_count'),
    )
    supplier_id = mapped_column("supplier_id", VARCHAR(12), ForeignKey("users.inn"), primary_key=True)
...
class Rnp(Base):
    __tablename__ = "rnp"
    supplier_inn = mapped_column("supplier_inn", VARCHAR(12), ForeignKey("users.inn"), primary_key=True)
    supplier_kpp = mapped_column("supplier_kpp", VARCHAR(9), nullable=True)
...
class MspRoster(Base):
    ...
    include_date = mapped_column("include_date", Date, nullable=False)
    organization_type = mapped_column("organization_type", EnumType(OrganizationType), nullable=False)
...
class EgrulInfo(Base):
    ...
    entity_state_record_date = mapped_column("entity_state_record_date", Date, nullable=True)
    ...
    entity_wo_attorney_type = mapped_column("entity_wo_attorney_type", TEXT, nullable=True)
    entity_wo_attorney_position = mapped_column("entity_wo_attorney_position", TEXT, nullable=True)
...
class BalanceCapitalChange1(Base):
    ...
    unappropriated_balance = mapped_column("unappropriated_balance", TEXT, nullable=True)
